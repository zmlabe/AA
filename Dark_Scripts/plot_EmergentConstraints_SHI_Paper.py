...
if variable == 'THICK':
    plt.axvspan(diffe,diffr,alpha=1,color='darkgrey',clip_on=False)
    plt.axhspan(diffshe,diffshr,alpha=1,color='darkgrey',clip_on=False,linewidth=0)
    plt.plot(xaxis,linetrend,linewidth=2,color='w')
    
    color = cmocean.cm.thermal(np.linspace(0.0,1,len(runnames)))
    for i,c in zip(range(len(runnames)),color):
        plt.scatter(meanPOL[i],meanSHI[i],color=c,s=90,
    ...
    for i,c in zip(range(len(runnames_E3SM)),color):
        plt.scatter(meanPOL_E3SM[i],meanSHI_E3SM[i],color=c,s=90,
    ...
    for i,c in zip(range(len(runnames_AMIP)),color):
        plt.scatter(POLAMIP_ALL[i],SHIAMIP_ALL[i],color=c,s=90,
                    label=r'\textbf{%s}' % runnames_AMIP[i],zorder=11,
                    clip_on=False,marker='v',edgecolor='w',linewidth=0.35)   
